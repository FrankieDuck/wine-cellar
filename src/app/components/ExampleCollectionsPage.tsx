...
const ExampleCollectionsPage: React.FC<ExampleCollectionsPageProps> = ({ type }) => {
    ...
    useEffect(() => {
        const fetchWines = async () => {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_CLOUD_API_URL}/wines/type/Rosé?p=${currentPage}`);
    ...
    const handleAddToCollection = async (wine: WineDataMax) => {
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_CLOUD_API_URL}/personal_collection`, wine);
